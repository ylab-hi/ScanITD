...
from .helper import obtain_sa_query_seq_from_ra
from .helper import format_sa_tag
from .helper import parse_target_genomic_coordinates
from .helper import get_insertion_reference_pos
from .helper import obtain_depth_given_genomic_position
from .helper import self_loop_checker
from .sr_resuer import update_tdup_ao

from scanitd.base import Read
    ...
    def iter_bam(self):
        ...
        for _region in self.regions:
            for read in self.in_bam_object.fetch(region=_region):
                ...
                if (
                    read.has_tag("SA")
                    and read.mapping_quality >= self.mapq_cutoff
                    ...
                    chrm_sa, pos_sa, strand_sa, cigar_sa, mapq_sa, nm_sa = (
                        format_sa_tag(sa_string)
                    )
...
def scan_itd(
    ...
    for _region in regions:
        try:
            for pileup_column in bam_object.pileup(
                region=_region, stepper="all", truncate=True
            ):
                    ...
                    if position_of_pileup_site and read.mapping_quality >= mapq_cutoff:
                        ...
                        if "S" in cigar_ra and "H" not in cigar_ra:
                            ...
                            if read_name not in tdup_anchors:
                                ...
                                if read_mode == MappingMode.MS:
                                    softclipped_sequence = read_obj.query_sequence[
                            ...
                            # deal with TDUP anchors
                                ...
                                tdup_seq = genome_fasta[chrm_ra][
                                    tdup_ref_start:tdup_ref_end
                                ].seq

                                tdup_id = chrm_ra, tdup_ref_start, tdup_size, tdup_seq

                                ref_allele = genome_fasta[chrm_ra][
                                    tdup_ref_start : tdup_ref_start + 1
                                ].seq

                                tdup_allele_dict[tdup_id] = (ref_allele, "TDUP")

                                # left softcliping, right softclipping
                                left_softclipped_sequence = read_obj.query_sequence[
                                    : read_obj.lt_soft_len
                                ]
                                right_softclipped_sequence = read_obj.query_sequence[
                                    -read_obj.rt_soft_len :
                                ]

                                if tdup_id not in tdup_anchors_sequences:
                                    tdup_anchors_sequences[tdup_id] = [
                                        left_softclipped_sequence,
                                        right_softclipped_sequence,
                                    ]
                                else:
                                    _left_soft_seq, _right_soft_seq = (
                                        tdup_anchors_sequences[tdup_id]
                                    )
                                    if len(left_softclipped_sequence) > len(
                                        _left_soft_seq
                                    ):
                                        tdup_anchors_sequences[tdup_id][0] = (
                                            left_softclipped_sequence
                                        )
                                    if len(right_softclipped_sequence) > len(
                                        _right_soft_seq
                                    ):
                                        tdup_anchors_sequences[tdup_id][1] = (
                                            right_softclipped_sequence
                                        )
                        ...
                        if pileup_read.indel >= itd_length_cutoff:
                            ...
                            if re.search(rf"\d+M{insertion_size}I\d+M", cigar_ra):
                                left_seq_from_genome = genome_fasta[chrm_ra][
                                    (
                                        pileup_column.reference_pos - insertion_size + 2
                                    ) : pileup_column.reference_pos + 1
                                ].seq

                                right_seq_from_genome = genome_fasta[chrm_ra][
                                    pileup_column.reference_pos + 1 : (
                                        pileup_column.reference_pos + insertion_size
                                    )
                                ].seq

                                insertion_seq_in_read = seq_ra[
                                    position_of_pileup_site + 1 : (
                                        position_of_pileup_site + insertion_size + 1
                                    )
                                ]
                                ...
                                if (
                                    is_dup
                                    and get_insertion_reference_pos(
                                        cigar_ra, pos_ra, insertion_size
                                    )
                                    == pileup_column.reference_pos
                                ):
                                    tdup_ref_start = (
                                        pileup_column.reference_pos - left_shift
                                    )
                                    tdup_id = (
                                        chrm_ra,
                                        tdup_ref_start,
                                ...
                                # Novel sequence insertion
                                    ...
                                    ref_allele = genome_fasta[chrm_ra][
                                        pileup_column.reference_pos : pileup_column.reference_pos
                                        + 1
                                    ].seq
                                    alt_allele = seq_ra[
                                        position_of_pileup_site : position_of_pileup_site
                                        + insertion_size
                                    ]
    ...
    for tdup_id in tdup_ao:
        ...
        event_list.append(
            Event.new("TDUP", tdup_id, new_ao, depth, ref_allele, alt_allele)
        )

    for ins_id in ins_ao:
        _chrom, _ref_start, _event_size, _event_seq = ins_id
        ao = ins_ao[tdup_id]
        depth = obtain_depth_given_genomic_position(bam_object, _chrom, _ref_start)
    ...
    sorted_event_list = sorted(
        event_list, key=lambda event: (event.chrom, event.ref_start)
    )
    bam_object.close()
